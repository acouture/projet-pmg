		  ...
		  calc_t radius, unsigned natoms, unsigned offset)
{
    unsigned index = get_global_id (0);
    calc_t p = pos[index];
    calc_t sp = speed[index];
    unsigned c = index / offset;
    unsigned i = index % offset;
    calc_t mi = min[c] + radius;
    calc_t ma = max[c] - radius;

    sp *= ((p + sp < mi) | (p + sp > ma)) ? -0.85f : 1.0f;
    speed[c * offset + i] = sp;    
}
...
void scan(__global int *box_buff, __global int *calc_offset_buff, unsigned begin, unsigned end, __local int* local_sumz)
{
  ...
  unsigned num_groups = get_num_groups(0);
